...
<html>
  ...
  <body >
  		<div id="main">
  			...
  			<div id="body_wrapper">
  				<form action="ExpEditAction" method="post"  >
  					<div id="op">
  						<div class="noavatar">
  							<h3>快递费修改</h3>
  						</div>
  						<%-- <s:iterator value="proList" id="proProname" status="S">
  						<s:iterator value = "updateList" id = "updateSto1">
  							<s:if test="#updateSto1.proname.id == #proProname.id"><s:property value="#proProname.province"/></s:if>
  						</s:iterator>
  						</s:iterator> --%>
  						<s:iterator value = "updateList" id = "updateSto">
	  						<table class="gridtable"  >
		  						<thead><tr><td width="80" >省份</td><td>首重公斤</td><td>首重价格</td><td>续重价格</td><td>保留方法</td></tr></thead>
		  							<%-- <input type="hidden" name="sto.id" value="<s:property value="#updateSto.id"/>"/> --%>
		  							<tr><td><s:property value="#updateSto.proname.province"/></td>
		  							<td><input type="text" name="sto.firstWeight" value="<s:property value="#updateSto.firstWeight"/>" /></td>
		  							<td><input type="text" name="sto.firstPrice" value="<s:property value="#updateSto.firstPrice"/>" /></td>
		  							<td><input type="text" name="sto.secPrice" value="<s:property value="#updateSto.secPrice"/>" /></td>
		  							<td>
		  								<select name = "Sto.retention">
		  									<option selected value="<s:if test ="#updateSto.retention == 1">1</s:if><s:if test ="#updateSto.retention == 2">2</s:if>">
		  									<s:if test ="#updateSto.retention == 1">四舍五入</s:if><s:if test ="#updateSto.retention == 2">向上取整</s:if></option>
		  									<option value="<s:if test ="#updateSto.retention == 1">2</s:if><s:if test ="#updateSto.retention == 2">1</s:if>">
		  									<s:if test ="#updateSto.retention == 1">向上取整</s:if><s:if test ="#updateSto.retention == 2">四舍五入</s:if></option>
		  								</select>
		  							</td>
  						...
  						<div class="opa">
  							<input type="hidden" name="edit" value = "updateimpl"/>
